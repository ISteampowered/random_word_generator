...
class Prwg:
    ...
    @staticmethod
    def generate_word(file_in='', object_in=None, precise_word_length=-1, min_word_length=0, print_to_terminal=False, **extra_files):
        """
        ...
        :param object_in: loads data from the given SaveAndLoad object
        ...
        my_obj = Prwg.__load_saveandload_object(file_in=file_in, object_in=object_in)
    ...
    @staticmethod
    def generate_sentence(sentence_data_file, file_in='', object_in=None, print_to_terminal=False, sentence_length=17,
        ...
        if sentence_data_file == '':
            ...
            sentence = ''
            sentence_obj = Prwg.__load_saveandload_object(file_in=file_in, object_in=object_in)
    ...
    @staticmethod
    def __load_saveandload_object(file_in='', object_in=None):
        if file_in == '' and object_in is None:
            raise IOError("no data input given")
        if file_in != '':
            my_obj = SaveAndLoad(file_loc=file_in)
        elif isinstance(object_in, SaveAndLoad):
            my_obj = object_in
        else:
            raise TypeError("object_in is not a SaveAndLoad object")
