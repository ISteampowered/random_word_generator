...
over_arching_group.add_argument('-s' '--sentence', action='store_true', default=False,
...
parser.add_argument('-t', '--sentence_length',  default=17, type=int,
                    help='set a precise sentence length, only required if the -s option is used')
parser.add_argument('-f', '--sentence-file', help='sentence file location, only needed if -s option is used')
parser.add_argument('-w', '--sentence-starting-word-length', default=5, type=int,
                    help='starting word length, only needed if the -s option is used')

group2 = over_arching_group.add_mutually_exclusive_group()
group2.add_argument('-a', '--pseudo-random-word-amount', type=int, default=1,
                    help='generates a set amount of pseudo random words')
group2.add_argument('-r', '--random-amount', type=int, nargs=2, default=[1, 1],
                    help='a random amount of words between the bounds given ([a,b[)')

group = over_arching_group.add_mutually_exclusive_group()
group.add_argument('-m', '--minimum-word-length' , type=int, default=0, help='set a minimum for the word length')
group.add_argument('-l', '--word-length', type=int, default=-1, help='set a precise length for the pseudo random word')
